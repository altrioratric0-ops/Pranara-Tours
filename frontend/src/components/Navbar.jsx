      ...
      <div className="navbar-shell">
        ...
        <div className={`navbar-menu${menuOpen ? ' open' : ''}`}>
          ...
          <div className="nav-group">
            <button type="button" className={`nav-link${activeMenu === 'Destinations' ? ' active' : ''}`}>
              Destinations
            </button>
            <div className="nav-submenu">
              {DESTINATION_LINKS.map((item) => (
                <Link key={item.path} to={item.path} className="nav-submenu-link" onClick={() => setMenuOpen(false)}>
                  {item.label}
                </Link>
              ))}
            </div>
